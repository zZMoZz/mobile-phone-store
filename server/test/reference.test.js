...
describe('categories & brands API', () => {
  ...
  it('rejects a category missing a name', async () => {
    const res = await api.post('/api/categories').send({ name_en: 'Only EN' });
    expect(res.status).toBe(400);
  });

  it('rejects a case-insensitive duplicate name', async () => {
    await api.post('/api/brands').send({ name_en: 'Nokia', name_ar: 'نوكيا' });
    const dup = await api.post('/api/brands').send({ name_en: '  nokia ', name_ar: 'نوكيا' });
  ...
  it('deletes an unused brand', async () => {
    const created = await api.post('/api/brands').send({ name_en: 'Temp Brand', name_ar: 'ماركة مؤقتة' });
    const del = await api.delete(`/api/brands/${created.body.id}`);
    expect(del.status).toBe(204);
  });

  it('blocks deleting a category still used by a product', async () => {
    const cat = await api.post('/api/categories').send({ name_en: 'In Use', name_ar: 'مستخدمة' });
    const brand = await api.post('/api/brands').send({ name_en: 'In Use B', name_ar: 'مستخدمة ب' });
    await api.post('/api/products').send({
      name: 'Uses Category',
      buying_price: 10,
      selling_price: 20,
    ...
    const del = await api.delete(`/api/categories/${cat.body.id}`);
    expect(del.status).toBe(409);
  });

  it('reassigns products to another category when deleting with moveTo', async () => {
    const from = await api.post('/api/categories').send({ name_en: 'From Cat', name_ar: 'من' });
    const to = await api.post('/api/categories').send({ name_en: 'To Cat', name_ar: 'إلى' });
    const brand = await api.post('/api/brands').send({ name_en: 'Reassign B', name_ar: 'علامة نقل' });
    ...
    const del = await api.delete(`/api/categories/${from.body.id}`).query({ moveTo: to.body.id });
    expect(del.status).toBe(204);
    ...
    const moved = await api.get(`/api/products/${product.body.id}`);
    expect(moved.body.category_id).toBe(to.body.id);
  });

  it('list reports the product_count for each record', async () => {
    const cat = await api.post('/api/categories').send({ name_en: 'Counted', name_ar: 'معدودة' });
    const brand = await api.post('/api/brands').send({ name_en: 'Counted B', name_ar: 'علامة معدودة' });
    await api.post('/api/products').send({
  ...
  it('always has a protected Generic category and brand that cannot be deleted', async () => {
    for (const kind of ['categories', 'brands']) {
      const list = await api.get(`/api/${kind}`);
      ...
      const del = await api.delete(`/api/${kind}/${generic.id}`);
