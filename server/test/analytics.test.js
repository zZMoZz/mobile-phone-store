...
describe('analytics, settings, and data export', () => {
  ...
  it('also copies the backup to the configured external folder', async () => {
    ...
    try {
      await request(app).put('/api/settings').send({ backup_dir: externalDir });
      const res = await request(app).post('/api/backup');
      expect(res.status).toBe(200);
      ...
      await request(app).put('/api/settings').send({ backup_dir: '' });
    }
  });
  ...
  it('exports products and transactions as CSV', async () => {
    const products = await request(app).get('/api/export/products.csv');
    expect(products.status).toBe(200);
    expect(products.headers['content-type']).toMatch(/text\/csv/);
    expect(products.text).toContain('Name');

    const txns = await request(app).get('/api/export/transactions.csv');
