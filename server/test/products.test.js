...
describe('products API', () => {
  ...
  it('rejects a duplicate name (case-insensitive)', async () => {
    await request(app).post('/api/products').send(validProduct({ name: 'Unique Widget' }));
    const dup = await request(app).post('/api/products').send(validProduct({ name: '  unique widget ' }));
  ...
  it('looks up a product by barcode', async () => {
    const res = await request(app).get('/api/products/lookup').query({ barcode: 'A1' });
  ...
  it('restocks an existing product by id', async () => {
    ...
    const res = await request(app).post(`/api/products/${id}/add-stock`).send({ quantity: 6 });
    expect(res.status).toBe(200);
    expect(res.body.quantity).toBe(10); // 4 + 6
  });

  it('rejects restock with a non-positive quantity', async () => {
    const created = await request(app).post('/api/products').send(validProduct({ name: 'No Negative Restock' }));
    const res = await request(app)
  ...
  it('returns 404 when restocking a missing product', async () => {
    const res = await request(app).post('/api/products/999999/add-stock').send({ quantity: 1 });
  ...
  it('restocking at a new cost sets the weighted-average buying price', async () => {
    ...
    const res = await request(app).post(`/api/products/${id}/add-stock`).send({ quantity: 10, unit_cost: 7000 });
    expect(res.status).toBe(200);
  ...
  it('restocking without a cost leaves the buying price unchanged', async () => {
    ...
    const res = await request(app).post(`/api/products/${id}/add-stock`).send({ quantity: 5 });
    expect(res.status).toBe(200);
  ...
  it('records a purchase transaction when created with initial stock', async () => {
    ...
    const detail = await request(app).get(`/api/products/${created.body.id}`);
  ...
  it('records a purchase transaction when restocking by id', async () => {
    ...
    await request(app).post(`/api/products/${id}/add-stock`).send({ quantity: 5 });
  ...
  it('updates and deletes a product', async () => {
    const created = await request(app).post('/api/products').send(validProduct({ name: 'Temp' }));
    const id = created.body.id;
  ...
  it('lists with search, sort, and pagination', async () => {
    ...
    await request(app).post('/api/products').send(validProduct({ name: 'Samsung Charger', selling_price: 80 }));
    await request(app).post('/api/products').send(validProduct({ name: 'Samsung Earbuds', selling_price: 300 }));

    const search = await request(app).get('/api/products').query({ search: 'Samsung' });
    ...
    const paged = await request(app).get('/api/products').query({ pageSize: 1, page: 1 });
  ...
  it('filters by price range and stock status', async () => {
    const inRange = await request(app).get('/api/products').query({ minPrice: 200, maxPrice: 400 });
    expect(inRange.body.items.every((p) => p.selling_price >= 200 && p.selling_price <= 400)).toBe(true);

    const inStock = await request(app).get('/api/products').query({ inStock: 'true' });
    expect(inStock.body.items.every((p) => p.quantity > 0)).toBe(true);
  });

  it('filters by quantity range', async () => {
    await request(app).post('/api/products').send(validProduct({ name: 'Low Qty', selling_price: 5, quantity: 1 }));
    await request(app).post('/api/products').send(validProduct({ name: 'High Qty', selling_price: 5, quantity: 50 }));

    const ranged = await request(app).get('/api/products').query({ minQty: 10, maxQty: 100 });
  ...
  it('includes a low-stock count in the summary', async () => {
    await request(app).post('/api/products').send(validProduct({ name: 'Almost Out', quantity: 1 }));
    const res = await request(app).get('/api/products/summary');
    expect(res.status).toBe(200);
  ...
  it('summary reflects the active filters', async () => {
    ...
    await request(app).post('/api/products').send(validProduct({ name: `${tag} A`, quantity: 4, buying_price: 10 }));
    await request(app).post('/api/products').send(validProduct({ name: `${tag} B`, quantity: 6, buying_price: 10 }));

    const filtered = await request(app).get('/api/products/summary').query({ search: tag });
    ...
    const all = await request(app).get('/api/products/summary');
  ...
  it('filters to low-stock products and matches the summary count', async () => {
    ...
    const filtered = await request(app).get('/api/products').query({ lowStock: 'true', pageSize: 200 });
    expect(filtered.status).toBe(200);
  ...
  it('uploads a product image and sets image_path', async () => {
    const created = await request(app).post('/api/products').send(validProduct({ name: 'With Image' }));
    const res = await request(app)
  ...
  it('includes empty history on product detail', async () => {
    const created = await request(app).post('/api/products').send(validProduct({ name: 'Detail Test' }));
