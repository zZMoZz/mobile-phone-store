...
describe('option lists API', () => {
  let api;
  let cleanup;
  beforeAll(async () => { ({ api, cleanup } = await setupTestApp()); });
  afterAll(() => cleanup());
  ...
  it('rejects a list with a missing name', async () => {
    const res = await api.post('/api/option-lists').send({ name_en: 'Only EN', options: [] });
  ...
  it('updates options and deletes', async () => {
    ...
    const del = await api.delete(`/api/option-lists/${created.body.id}`);
